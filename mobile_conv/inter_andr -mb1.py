...
import datetime
MODEL_NAME_SUFFIX_IMG_NAMING="_mb1-ssd_11-18"
...
net = create_mobilenetv1_ssd(len(CLASSES_BEAUTY), is_test=True)
import re
if(path_type=='1'):
    print("loading state dict...")
    net.load_state_dict(torch.load(PATH_TO_WEIGHTS))
elif(path_type=='2'):
    print("extracting from parallel state dict...")
    state_dict = torch.load(PATH_TO_WEIGHTS)
    state_dict = {re.match(r"module\.module\.(.+)", k).group(1): v for k, v in state_dict.items() if (k.startswith("module.module"))}
    net.load_state_dict(
                       state_dict)#
else:
    print("loading net")
...
torch.onnx.export(
        net,
        x,
        MODEL_NAME_SUFFIX_IMG_NAMING+".onnx",#'onn.onnx',
        export_params=True,
        opset_version=11,
